...
_tuple(b::Bond1ParticleFcn) = (b.k, b.l, b.m)
Base.vec(b::Bond1ParticleFcn) = [b.k, b.l, b.m]

function Bond1ParticleFcn(klm::Union{AbstractVector, Tuple}, TI=Int16)
   @assert length(klm) == 3
   return Bond1ParticleFcn(TI(klm[1]), TI(klm[2]), TI(klm[3]))
end
...
"""
struct BondAList{TI}
   i2zklm::Vector{Bond1ParticleFcn{TI}}
   zklm2i::Dict{Bond1ParticleFcn{TI}, TI}
   # firstz::Vector{TI}
end
...
Dict(alist::BondAList{TI}) where {TI} =
      Dict( "__id__" => "SHIPs_BondAList",
            "TI"     => string(TI),
            "i2zklm" => vec.(alist.i2zklm))
BondAList(D::Dict) = BondAList(
      Bond1ParticleFcn.(D["i2zklm"],
                        Meta.eval(Meta.parse(D["TI"])))  )
==(al1::BondAList, al2::BondAList) = (al1.i2zklm == al2.i2zklm)
# ------------------------------------------------------------------------

Base.length(alist::BondAList) = length(alist.i2zklm)
Base.getindex(alist::BondAList, i::Integer) = alist.i2zklm[i]
Base.getindex(alist::BondAList, zklm::Bond1ParticleFcn) = alist.zklm2i[zklm]

# TODO: rename this to alloc_B ???
alloc_A(alist::BondAList, T=Float64) = zeros(Complex{T}, length(alist))

function BondAList(zklmlist::AbstractVector)
   # sort the tuples - by z, then k, then l, then m
   INT = _inttype(zklmlist[1])
   i2zklm = Bond1ParticleFcn.( sort(_tuple.(zklmlist)), INT )
   # create the inverse mapping
   zklm2i = Dict{Bond1ParticleFcn{INT}, INT}()
   for i = 1:length(i2zklm)
      zklm2i[i2zklm[i]] = INT(i)
   end
   # TODO: z dependence
