...
one-particle basis.
"""
mutable struct XScal1pBasis{VSYM, ISYMS, T, TT, TJ, NI} <: OneParticleBasis{T}
   P::TransformedPolys{T, TT, TJ}
   spec::Vector{NamedTuple{ISYMS, NTuple{NI, Int}}}
   coeffs::Matrix{T}
end

# ** need a better convenience function to construct these 
# xscal1pbasis(varsym::Symbol, idxsyms::NTuple{NI, Symbol}, 
#              args...; kwargs...) where {NI} = 
#       XScal1pBasis(varsym, idxsyms,  
...
function XScal1pBasis(varsym::Symbol, ISYMS::NTuple{NI, Symbol}, 
   ...
   return XScal1pBasis{varsym, ISYMS, T, TT, TJ, NI}(P, spec, Matrix{T}(undef, (0,0)))
end
...
==(P1::Scal1pBasis, P2::Scal1pBasis) = _allfieldsequal(P1, P2)
...
function indexrange(basis::XScal1pBasis) 
   ISYMS = _idxsyms(basis)
   rgs = ntuple(i -> (i == 1 ? (1:length(basis.P)) : (1:5)), length(ISYMS))
   return NamedTuple{ISYMS}(rgs)
end
