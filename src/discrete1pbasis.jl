...
Base.iterate(list::SList) = (list.list[1],1)
Base.iterate(list::SList{N, T}, state::Int) where {N,T} = (state >= N ? nothing : (list.list[state+1],state+1))
...
indexrange(basis::Categorical1pBasis) = Dict( _isym(basis) => [ val2i(basis.categories, val) for val in basis.categories.list] )

isadmissible(b, basis::Categorical1pBasis) = (_idx(b, basis) in basis.categories)

degree(b, basis::Categorical1pBasis, args...) = 0

get_index(basis::Categorical1pBasis, b) = val2i(basis.categories, _idx(b, basis) )
