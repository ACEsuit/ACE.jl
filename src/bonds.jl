...
cutoff_env(env::CylindricalBondEnvelope) = sqrt(env.rcut^2 + (env.λ * env.r0cut + env.zcut)^2)
...
cutoff_env(env::EllipsoidBondEnvelope) = env.zcut + env.floppy * env.λ * env.rcut 
